...
func Build(inputDir string, outputFileName string, pkgName string) error {
	...
	if err := merger.ParseDir(inputDir, outputFileName+".go"); err != nil {
		return err
	}
	...
	for _, decl := range funcs {
		...
		verb := strings.ToUpper(split[0])

		path := strings.Split(strings.ReplaceAll(name, "_", "/"), "routes/")[1]

		f := Code(Qual("e", verb).Call(List(Lit(path), Id(funcName))))
	...
	if err := os.WriteFile(outputFileName+".go", []byte(header+merged+"\n"+codeAsString), 0666); err != nil {
		return err
	}
